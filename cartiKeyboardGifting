...
local currentGiftRecipient = ""
local pendingGiftNotificationRecipient = ""
local pendingGiftNotificationExpiresAt = 0
local pendingGiftNotificationLockedText = ""
local giftNotificationWatcherStarted = false
local cachedGeneralNotificationTemplate = nil
local generalNotificationHookStarted = false
...
local function setGeneralNotificationText(frame, text)
    if not frame then
        return false
    end
...
local function getGeneralNotificationFrame()
    local playerGui = localPlayer:FindFirstChildOfClass("PlayerGui")
    local mainFrame = playerGui
        and playerGui:FindFirstChild("GeneralNotificationGui")
        and playerGui.GeneralNotificationGui:FindFirstChild("MainFrame")

    return mainFrame, mainFrame and mainFrame:FindFirstChild("GeneralNotificationFrame")
end

local function cacheGeneralNotificationTemplate()
    local _, frame = getGeneralNotificationFrame()
    ...
    if frame then
        ...
        if ok and clone then
            clone.Name = "CartiHubCachedGeneralNotificationFrame"
            cachedGeneralNotificationTemplate = clone
        end
    end
end

local function rewriteGiftNotificationIfNeeded()
    if pendingGiftNotificationRecipient == "" or os.clock() > pendingGiftNotificationExpiresAt then
        pendingGiftNotificationRecipient = ""
        pendingGiftNotificationLockedText = ""
        return false
    end

    local desiredText = pendingGiftNotificationLockedText ~= "" and pendingGiftNotificationLockedText
        or ("Gift sent to " .. tostring(pendingGiftNotificationRecipient) .. "!")
    local _, frame = getGeneralNotificationFrame()
    local notificationText = frame and frame:FindFirstChild("Text")

    if notificationText and notificationText:IsA("TextLabel") then
        local currentText = tostring(notificationText.Text or "")
        local lowerText = currentText:lower()
        local lowerDesired = desiredText:lower()

        if lowerText:find("gift cancelled", 1, true) or lowerText == lowerDesired then
            if setGeneralNotificationText(notificationText.Parent, desiredText) then
                foundationDebug("gift_notice", "rewrote exact general notification label")
                return true
            end
        end
    end
...
local function startGiftNotificationRewriteWatcher()
    if giftNotificationWatcherStarted then
        return
    end

    giftNotificationWatcherStarted = true

    task.spawn(function()
        while true do
            cacheGeneralNotificationTemplate()
            rewriteGiftNotificationIfNeeded()
            task.wait(0.05)
        end
    end)
end

local function maybeOverrideGiftCancelledLabel(label)
    if pendingGiftNotificationRecipient == "" or os.clock() > pendingGiftNotificationExpiresAt then
        return
    end
    ...
    local lowerText = tostring(label.Text or ""):lower()
    if lowerText:find("gift cancelled", 1, true) then
        label.Text = pendingGiftNotificationLockedText ~= "" and pendingGiftNotificationLockedText
            or ("Gift sent to " .. tostring(pendingGiftNotificationRecipient) .. "!")
        label.TextColor3 = Color3.fromRGB(85, 255, 127)
        foundationDebug("gift_notice_hook", "overrode newly spawned gift cancelled label")
    end
end

local function hookGeneralNotificationMainFrame()
    if generalNotificationHookStarted then
        return
    end

    generalNotificationHookStarted = true

    task.spawn(function()
        local connectedMainFrame = nil
        local mainConn = nil

        while true do
            local mainFrame = getGeneralNotificationFrame()

            if typeof(mainFrame) == "Instance" and mainFrame ~= connectedMainFrame then
                connectedMainFrame = mainFrame
                ...
                mainConn = mainFrame.DescendantAdded:Connect(function(descendant)
                    if descendant:IsA("TextLabel") and descendant.Name == "Text" then
                        task.defer(function()
                            maybeOverrideGiftCancelledLabel(descendant)
...
local function fireGiftSentTopNotification(recipient)
    if recipient == nil or tostring(recipient) == "" then
        return false
    end

    local success, err = pcall(function()
        local event = game:GetService("ReplicatedStorage").Packages._Index["littensy_remo@1.5.3"].remo.container.ShopUpdate
        firesignal(event.OnClientEvent, {
            purchases = {},
            nextRestockTime = os.time() + 300,
            items = {
                Mysterious = "DonutShield",
                Common = "ChocoBar",
                Uncommon = "MuffinHat",
                Rare = "MarshmallowHelmet",
            },
            active = true,
            restock = true,
            stocks = {
            ...
            serverTime = os.time(),
            mysteriousRarity = "Epic",
        })
    end)

    if not success then
        foundationDebug("gift_notice_fire", "failed to fire ShopUpdate: " .. tostring(err))
        return false
    end

    pendingGiftNotificationRecipient = tostring(recipient)
    pendingGiftNotificationLockedText = "Gift sent to " .. tostring(recipient) .. "!"
    pendingGiftNotificationExpiresAt = os.clock() + 10
    foundationDebug("gift_notice_fire", "fired ShopUpdate notification for gift recipient")
    return true
end
...
local function showFoundationOverlayPromptReplica()
    ...
    headerPrice.Position = UDim2.new(1, -36, 0, 16)
    ...
    local headerRobuxAmount = makeText(headerPrice, "Amount", tostring(localRobuxAmount), UDim2.new(1, -25, 1, 0), UDim2.fromOffset(21, -1), 14, textColor, boldFont)
    ...
    buyButton.MouseButton1Click:Connect(function()
        ...
        if currentGiftRecipient ~= nil and tostring(currentGiftRecipient) ~= "" then
            pendingGiftNotificationRecipient = tostring(currentGiftRecipient)
            pendingGiftNotificationExpiresAt = os.clock() + 10
            pendingGiftNotificationLockedText = "Gift sent to " .. tostring(currentGiftRecipient) .. "!"
        end
...
local function pickPromptGiftRecipient(root)
    local exactInput = localPlayer
        and localPlayer:FindFirstChildOfClass("PlayerGui")
        and localPlayer.PlayerGui:FindFirstChild("SpeedGameUI")
        and localPlayer.PlayerGui.SpeedGameUI:FindFirstChild("Modals")
        and localPlayer.PlayerGui.SpeedGameUI.Modals:FindFirstChild("GiftToPlayerModal")
        and localPlayer.PlayerGui.SpeedGameUI.Modals.GiftToPlayerModal:FindFirstChild("MainFrame")
        and localPlayer.PlayerGui.SpeedGameUI.Modals.GiftToPlayerModal.MainFrame:FindFirstChild("TextboxFrame")
        and localPlayer.PlayerGui.SpeedGameUI.Modals.GiftToPlayerModal.MainFrame.TextboxFrame:FindFirstChild("UsernameInput")

    if exactInput and exactInput:IsA("TextBox") then
        local exactText = normalizeText(exactInput.Text)
        if exactText ~= "" then
            return exactText
        end
    end
    ...
    for _, descendant in ipairs(root:GetDescendants()) do
        if descendant:IsA("TextBox") then
            ...
            if text ~= "" then
                ...
                if lowerName:find("recipient", 1, true) then
                    score += 1000
                end
                if lowerName:find("username", 1, true) or lowerName:find("user", 1, true) then
                    score += 700
                end
                if lowerName:find("name", 1, true) then
                    score += 300
                end
                if fullName:find("gift", 1, true) then
                    score += 800
                end
                if fullName:find("recipient", 1, true) or fullName:find("username", 1, true) then
                    score += 600
                end
...
clearSuppressedRealPurchaseOverlays = function()
    ...
    _G.CartiHubFoundationGiftRecipient = ""
...
local function captureAndReplaceRealFoundationPrompt(candidate, allowFullScan)
    ...
    currentGiftRecipient = pickPromptGiftRecipient(root) or ""
    _G.CartiHubFoundationGiftRecipient = currentGiftRecipient
...
panelTitle.Text = "Enter how much robuxu want"
...
startGiftNotificationRewriteWatcher()
hookGeneralNotificationMainFrame()
